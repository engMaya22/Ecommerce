			...
			<div class="invoice_items">
	    		<table class="table table-borderless ">
	    				...
	    				@foreach ($invoice->items as $key => $item)
		    				<tr>
		    					...
		    					<td class="text-right">
		    						<form 
		    							method="POST" 
		    							action=" {{ route('user.sales.invoices.delete_item', ['user_id' => $user->id, 'invoice_id' => $invoice->id, 'item_id'=> $item->id]) }} ">
					              		@csrf
					              		@method('DELETE')
					...
					<tfoot>
						<th></th>
						<th>
							{{-- <button class="btn btn-info btn-sm"> <i class="fa fa-plus"></i> Add Product</button> --}}
 ...
 {{-- form to add Product --}}
  ...
  <div class="modal fade" id="newProduct" tabindex="-1" role="dialog" aria-labelledby="newProductLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
	 {!! Form::open(['route' => ['user.sales.invoices.add_item',['user_id'=> $user->id ,'invoice_id'=>$invoice->id]], 'method' => 'post']) !!}
       <div class="modal-content">
		...
		<div class="modal-body">
			<div class="form-group row">
				...
				<div class="col-sm-9">
					{{ Form::select('product_id', $products, null, [ 'class'=>'form-control', 'id' => 'product', 'placeholder' => 'Select Product' ]);	}}				<span class="text-danger">
					</span>
				</div>
			</div>	
			<div class="form-group row">
